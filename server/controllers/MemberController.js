...
exports.getCouponByMemberId = async (req, res) => {
  ...
  if (token) {
    ...
    if (memberId) {
      try {
        let usableCouponlist = await memberModel.getUsableCouponByMemberId(
          memberId
        );
        let UnusableCouponByMemberId =
          await memberModel.getUnusableCouponByMemberId(memberId);
        configController.sendJsonMsg(res, true, "", {
          usableCouponlist,
          UnusableCouponByMemberId,
        });
      } catch (error) {
  ...
  if (token) {
    ...
    if (memberId) {
      try {
        ...
        let saveRainbowCard = await memberModel.saveRainbowCard(
          memberId,
          rainbowCardId
        );
        configController.sendJsonMsg(res, true, "", {
